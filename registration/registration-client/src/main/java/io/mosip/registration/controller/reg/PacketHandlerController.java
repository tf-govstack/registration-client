...
import static io.github.tf-govstack.registration.constants.LoggerConstants.PACKET_HANDLER;
import static io.github.tf-govstack.registration.constants.RegistrationConstants.ACKNOWLEDGEMENT_TEMPLATE_CODE;
...
public class PacketHandlerController extends BaseController implements Initializable {
	...
	private ResponseDTO savePacket(Writer stringWriter, RegistrationDTO registrationDTO) {
		...
		if (response.getSuccessResponseDTO() != null
			...
			} catch (io.github.tf-govstack.kernel.core.exception.IOException ioException) {
				LOGGER.error("",ioException);
			} catch (RegBaseCheckedException regBaseCheckedException) {
